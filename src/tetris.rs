...
impl Tetris {
    ...
    pub fn tick(&mut self) {
        ...
        let translated_shape = &self.current_shape + Pos(0, 1);

        if self.is_out_of_bounds(&translated_shape) || self.is_colliding(&translated_shape) {
            // Make current shape fixed
            let new_fixed_shape: Shape = mem::replace(
            ...
            self.current_shape = translated_shape;
        }
    }

    pub fn shift_current_shape(&mut self, direction: Direction) {
        let translated_current_shape: Shape = &self.current_shape
            + match direction {
    ...
    pub fn rotate(&mut self) {
        let rotated_current_shape: Shape = self.current_shape.rotated();
...
mod tests {
    ...
    #[test]
    fn test() {
        let mut tetris = Tetris::new(5, 10);
        tetris.tick();
        tetris.tick();
        tetris.tick();
        println!("{:#?}", tetris);
